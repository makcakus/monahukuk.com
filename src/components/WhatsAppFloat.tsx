...
export function WhatsAppFloat({ locale }: { locale: string }) {
  ...
  const label = ARIA[locale] ?? ARIA.en;
  ...
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className="fixed bottom-6 end-6 z-50 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-emerald-900/30 hover:scale-105 hover:bg-[#1ebd5b] transition-transform print:hidden"
    >
      <svg
        viewBox="0 0 24 24"
        aria-hidden="true"
