...
- E-ticaret işletmesi; kayıt, sipariş, ödeme, kargo ve pazarlama aşamalarında kişisel veri işler.
- Kayıt sırasında **aydınlatma metni** sunulmalı; pazarlama amaçlı işleme için **ayrı bir açık rıza** alınmalıdır.
- Çerez bannerı zorunlu ve opsiyonel çerezleri ayırt etmeli, kullanıcıya seçim imkânı vermelidir.
- Google Analytics, Facebook Pixel, Hotjar gibi üçüncü taraf araçlar kişisel veri işler ve **yurt dışına aktarır**.
- Sipariş verisinin işlenmesinde **sözleşmenin kurulması ve ifası** hukuki sebebi yeterlidir; ödeme bilgilerinin saklanması ek güvence gerektirir.
...
### E-ticaret sitemde açık rıza almam şart mı?

Sipariş sürecinde müşteri verisi, **sözleşmenin kurulması ve ifası** hukuki sebebine dayanılarak açık rıza olmaksızın işlenebilir. Ancak **pazarlama amaçlı** veri işleme için aydınlatma metninden ayrı bir açık rıza alınması gerekir.

### Çerez bannerında neler bulunmalı?

Banner, sitede hangi çerezlerin kullanıldığını açıklamalı, **zorunlu ve opsiyonel çerezleri ayırt etmeli** ve kullanıcıya gerçek bir seçim imkânı sunmalıdır. Ayrıntılı çerez politikası ayrı bir sayfada yer almalıdır.

### Google Analytics kullanmak KVKK açısından risk mi?

Google Analytics, Facebook Pixel ve Hotjar gibi üçüncü taraf araçlar kişisel veri işler ve bu veriyi **yurt dışına aktarır**. Bu nedenle çerez politikasında açıkça listelenmeleri ve yurt dışına aktarım rejimine uygun bir hukuki dayanağa oturtulmaları gerekir.
