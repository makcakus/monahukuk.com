...
- Üç model, sözleşmenin tüm hükümlerini belirler: **on-premise** lisans, **SaaS** abonelik ve **özel yazılım geliştirme**.
- Özel yazılım, lisans sözleşmesinden çok **eser sözleşmesi** ile birlikte değerlendirilir; fikri mülkiyetin kimde kalacağı açıkça yazılmalıdır.
- Lisansın münhasır olup olmadığı, kullanıcı sayısı ve coğrafi sınırlar açıkça belirlenmelidir.
- SaaS sözleşmelerindeki en yaygın tuzak **otomatik yenileme** hükümleridir.
- SaaS'ta en kritik konu müşterinin **kendi verisi üzerindeki hakları ve çıkış (exit) hakkıdır**.
...
### SaaS sözleşmelerinde en sık hangi tuzakla karşılaşılır?

**Otomatik yenileme** hükümleri. Fesih bildirimi için öngörülen sürenin kaçırılması, sözleşmenin bir dönem daha kendiliğinden uzamasına yol açar. Sözleşmedeki fesih bildirim süresinin takvime işlenmesi gerekir.

### Özel geliştirilen yazılımın fikri mülkiyeti kime ait olur?

Bu, sözleşmede açıkça düzenlenmediği sürece uyuşmazlık doğuran bir konudur. Özel yazılım geliştirme, lisans sözleşmesinden çok bir **eser sözleşmesi** ile birlikte değerlendirilir; hakların müşteriye devredilip devredilmediği metinde açıkça yazılmalıdır.

### SaaS sağlayıcısıyla çalışırken verilerim güvende mi?

Sözleşmede açıkça korunmadıkça değil. SaaS sözleşmelerinde en önemli husus, müşterinin **kendi verisi üzerindeki hakları** ile sözleşme sona erdiğinde verisini dışa aktarabilme (çıkış) hakkının güvence altına alınmasıdır. Ayrıca hizmet seviyesi taahhütleri (SLA) ve sorumluluk sınırlama maddeleri dikkatle incelenmelidir.
