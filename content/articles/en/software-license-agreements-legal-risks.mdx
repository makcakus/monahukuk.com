...
- Intellectual property ownership (who owns the deliverables — does the customer have a usage license or full ownership?) must be clear.
...
### What is the most common trap encountered in SaaS agreements?

<b>Automatic renewal</b> clauses. Missing the notice period specified for termination causes the agreement to automatically extend for another term. The termination notice period in the contract needs to be entered into a calendar.

### Who owns the intellectual property of custom-developed software?

This is a matter that gives rise to disputes unless it is expressly regulated in the contract. Custom software development is assessed together with a <b>work contract (eser sözleşmesi)</b> rather than purely as a licence agreement; whether the rights are transferred to the customer must be explicitly stated in the text.

### Is my data safe when working with a SaaS provider?

Not unless it is expressly protected in the contract. The most important point in SaaS agreements is securing the customer's <b>rights over their own data</b> and the right to export (exit with) that data when the contract ends. In addition, service level commitments (SLAs) and liability limitation clauses must be examined carefully.
